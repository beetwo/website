...
let parallax = {
  ...
  add : function(key,object) {
    ...
    if(check){
      ...
      for(propName in this) if(propName === key) validKeyName = false
      if($.inArray(key, this.unusableNames) !== -1)validKeyName = false

      if(validKeyName){
        this[key] = parallaxPage(key,object);
        this[key].hide();
        this[key].page.css("position","absolute");
      } else alert("ERROR:'"+key+"' cannot be used as a page identifier");
    }
    return this;
  },
